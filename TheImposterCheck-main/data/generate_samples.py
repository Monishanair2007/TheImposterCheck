...
# -------------------------------------------------------------
def make_id_card(name="NEMAAN", doc_id="DL-8892140A", title="FEDERAL DRIVER LICENSE", is_tampered=False, face_alternate=False):
...
# 1. Genuine ID Photo
img_genuine = make_id_card("NEMAAN", "DL-8892140A", "STATE OF CALIFORNIA DRIVER LICENSE", is_tampered=False)
cv2.imwrite(str(OUT_DIR / "1_genuine_id_card.png"), img_genuine)

# 2. Forged Tampered ID
img_tampered = make_id_card("NEMAAN", "DL-8892140A", "STATE OF CALIFORNIA DRIVER LICENSE", is_tampered=True)
...
c1 = make_id_card("NEMAAN", "USA-PASS-449102", "UNITED STATES PASSPORT CARD", face_alternate=False)
c1_scaled = cv2.resize(c1, (440, 280))
multi_canvas[60:340, 60:500] = c1_scaled

# Place Card #2 (Driver License on bottom right)
c2 = make_id_card("NEMAAN", "DL-8892140A", "NATIONAL IDENTIFICATION CARD", face_alternate=False)
...
try:
    ...
    page1.insert_text((50, 90), "Subject: NEMAAN  |  Verification Status: KYC CLEARED", fontsize=11, color=(0.2, 0.5, 0.2))
...
# Matching Set: Monisha ID + Monisha Selfie
selfie_match = make_clean_selfie(is_alternate=False, text_label="SELFIE: NEMAAN (GENUINE MATCH)")
cv2.imwrite(str(OUT_DIR / "11_match_monisha_selfie.png"), selfie_match)
